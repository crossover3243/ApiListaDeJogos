...
if trim(cod_tipo_anexo)<>"" then
	...
	Set rsPesquisa = rsCursorOracle(CStr(sUsuario), _
  		 							...
  		 							CStr(sSistema), _
  									CStr(sModulo),_
									"TS.RBM_MANTER_TIPO_ANEXO_PREVIA.ObterTipoAnexoNovo", _
									vet_PL, _
									false )
...
<script language="javascript">
    ...
    function validacao() {
		...
		if (document.getElementById('trSubmodalidade').style.display == ""){
		...
		if (form01.ind_canais_digitais[0].checked == true){
...
function carregaSubModalidade(tipo) {
	if (form01.ind_tipo_reembolso.value=="" || tipo == "") {
	...
	cp.set_debug(false);
	cp.set_async(false);
	cp.call('../../rbm/asp/rbm0079f.asp', 'carregaSubModalidade', ExibeSubmodalidade, form01.ind_tipo_reembolso.value,"<%=ind_tipo_submodalidade%>","S"); 
}
//------------------------------------------------------------------
...
<BODY onload="atualizaModalidade(<%=ind_tipo_reembolso%>)">
...
<table border="0" width="100%" align="center">
	...
	<tr id="trSubmodalidade" style="display: none;">
		<td width="15%" nowrap class="label_right">Submodalidade&nbsp;</td>
...
<script type="text/javascript">
    ...
    function adicionarLinhaModalidade() {
        contador++;
        var tabela = document.getElementById("tblModalidades");
        var novaLinha = tabela.insertRow(-1);

        var td1 = novaLinha.insertCell(0);
        td1.className = "label_right";
        td1.innerHTML = "Modalidade";

        var td2 = novaLinha.insertCell(1);
        var htmlSelect = "<select name='ind_tipo_reembolso_" + contador + "' id='ind_tipo_reembolso_" + contador + "' onchange='carregaSubModalidadeDinamico(this, " + contador + ")'>";
        htmlSelect += "<option></option>";
        <% 
        set rsCombo = retornaCursor("tipo_reembolso","ind_tipo_reembolso","nome_tipo_reembolso","", "order by ind_tipo_reembolso") 
        rsCombo.MoveFirst
        do while not rsCombo.eof
            Response.Write "htmlSelect += \"<option value='" & rsCombo(0) & "'>" & rsCombo(1) & "</option>\";" & vbCrLf
            rsCombo.movenext
        loop
        %>
        htmlSelect += "</select>";
        td2.innerHTML = htmlSelect;

        var td3 = novaLinha.insertCell(2);
        td3.className = "label_right";
        td3.innerHTML = "Submodalidade";

        var td4 = novaLinha.insertCell(3);
        td4.id = "tdSubmodalidade_" + contador;
        td4.innerHTML = "<select name='ind_tipo_submodalidade_" + contador + "' id='ind_tipo_submodalidade_" + contador + "'><option></option></select>";
    }

    function carregaSubModalidadeDinamico(obj, index) {
        var valor = obj.value;
        if (valor === "") return;
        ...
        cp.call("../../rbm/asp/rbm0079f.asp", "carregaSubModalidade", function(resposta) {
            var alvo = document.getElementById("tdSubmodalidade_" + index);
            if (alvo) {
                alvo.innerHTML = resposta;
            }
        }, valor, "", "S");
    }
</script>
