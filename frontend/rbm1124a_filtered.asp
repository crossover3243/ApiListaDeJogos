...
<!-- BEGIN: Anti-duplicação de "Campo" + filtro de opções -->
<!-- Modelo com TODAS as opções vindas do servidor (não visível) -->
<select id="all_campos_model" style="display:none"><%=txt_combo_campos%></select>

<script type="text/javascript">
(function(){
    // Util: pega todas as opções possíveis do modelo oculto
    function getAllOptions(){
        var model = document.getElementById('all_campos_model');
        var out = [];
        if(!model) return out;
        for(var i=0;i<model.options.length;i++){
            var o = model.options[i];
            if(o.value !== "") out.push({value:o.value, text:o.text});
        }
        return out;
    }

    function getHiddenCampoInputs(){
        var inputs = document.getElementsByTagName('input');
        var arr = [];
        for(var i=0;i<inputs.length;i++){
            var n = inputs[i].getAttribute('name') || '';
            if(n.indexOf('cod_tipo_doc_campo_') === 0){
                arr.push(inputs[i]);
            }
        }
        return arr;
    ...
    function getSelectCampoElements(){
        var sels = document.getElementsByTagName('select');
        var arr = [];
        for(var i=0;i<sels.length;i++){
            var n = sels[i].getAttribute('name') || '';
            if(n.indexOf('cod_tipo_doc_campo_') === 0){
                arr.push(sels[i]);
            }
        }
    ...
    // Valores já usados (linhas antigas -> hidden + selects atuais)
    function getUsedValues(){
        var used = {};
        var h = getHiddenCampoInputs();
        for(var i=0;i<h.length;i++){
            var v = (h[i].value || '').trim();
            if(v) used[v] = true;
        }
        var s = getSelectCampoElements();
        for(var j=0;j<s.length;j++){
            var v2 = (s[j].value || '').trim();
            if(v2) used[v2] = true;
        }
        return used;
    }
    ...
    function getAvailableOptions(){
        ...
        for(var i=0;i<all.length;i++){
            if(!used[all[i].value]) avail.push(all[i]);
        }
        return avail;
    }

    // Popula um select com as opções passadas
    function populateSelect(selectEl, options, keepEmpty){
        if(!selectEl) return;
    ...
    // Reaplica filtro em todos os selects atuais
    function refilterAll(){
        var sels = getSelectCampoElements();
        for(var i=0;i<sels.length;i++){
            var sel = sels[i];
            var current = sel.value;
            var used = getUsedValues();
            // Permite manter o selecionado atual nesta linha
            var all = getAllOptions();
            var opts = [];
            for(var j=0;j<all.length;j++){
                if(all[j].value === current || !used[all[j].value]){
                    opts.push(all[j]);
                }
            }
            var hadEmpty = (sel.options.length && sel.options[0].value==="");
            populateSelect(sel, opts, true);
            sel.value = current;
            if(sel.value !== current){ sel.value = ""; } // caso o atual tenha sido removido
        }
    }

    // Monkey-patch do adicionarLinha original
    var _oldAdd = window.adicionarLinha;
    window.adicionarLinha = function(){
        var disponiveis = getAvailableOptions();
        if(disponiveis.length === 0){
            alert("Não há mais campos disponíveis para adicionar.");
            return false;
        }
        var beforeCount = getSelectCampoElements().length;
        var ret = (typeof _oldAdd === 'function') ? _oldAdd() : undefined;
        // após a linha ser criada, localizar o novo select
        var afterSels = getSelectCampoElements();
        var novoSelect = null;
        if(afterSels.length > 0){
            // heurística: o último select é o recém-criado
            novoSelect = afterSels[afterSels.length - 1];
        }
        if(novoSelect){
            // Preenche o novo select apenas com disponíveis
            populateSelect(novoSelect, disponiveis, true);
            if(novoSelect.options.length > 0){
                // seleciona vazio por padrão
                novoSelect.value = "";
            }
            // ao mudar, refiltra todos os demais
            if(!novoSelect.onchange){
                novoSelect.onchange = function(){ refilterAll(); };
            }
        }
        // Por segurança, refiltra todos
        refilterAll();
        return ret;
    };

    // Ao carregar, refiltra qualquer select presente
    if(document.readyState === 'complete' || document.readyState === 'interactive'){
        setTimeout(refilterAll, 0);
    } else {
        document.addEventListener('DOMContentLoaded', refilterAll);
    }
})();
</script>
<!-- END: Anti-duplicação -->
