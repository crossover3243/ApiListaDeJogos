...
<% If Len(txt_combo_campos) > 0 Then %>
<!-- BEGIN: Filtro dinâmico compatível com IE7 e IE8 -->
...
(function(){
    ...
    function getAllSelects(){
        ...
        for (var i = 0; i < all.length; i++) {
            if (all[i].name.indexOf("cod_tipo_doc_campo_") === 0) {
                arr.push(all[i]);
            }
        }
        return arr;
    ...
    function getSelectedValues(ignorar) {
        var usados = {};
        var selects = getAllSelects();
        for (var i = 0; i < selects.length; i++) {
            if (selects[i] === ignorar) continue;
            var val = selects[i].value;
            if (val) usados[val] = true;
        }
        return usados;
    }

    function populateFilteredOptions(sel) {
        var selected = sel.value;
        var all = getAllOptions();
        var usados = getSelectedValues(sel);
    ...
    function interceptarAdicao() {
        var oldAdd = window.adicionarLinha;
        window.adicionarLinha = function() {
            var todas = getAllOptions();
            var usados = getSelectedValues(null);
            var disponiveis = [];
            for (var i = 0; i < todas.length; i++) {
                if (!usados[todas[i].value]) disponiveis.push(todas[i]);
            }
            ...
            setTimeout(function() {
                ...
                if (selects.length > 0) {
                    ...
                    if (ultimo.attachEvent) {
                        ultimo.attachEvent("onchange", refiltrarTodos);
                    } else {
                        ultimo.addEventListener("change", refiltrarTodos, false);
                    }
                }
                refiltrarTodos();
            }, 50);
    ...
    function setupInicial(){
        ...
        for (var i = 0; i < selects.length; i++) {
            if (selects[i].attachEvent) {
                selects[i].attachEvent("onchange", refiltrarTodos);
            } else {
                selects[i].addEventListener("change", refiltrarTodos, false);
            }
        }
        refiltrarTodos();
        interceptarAdicao();
    ...
    if (document.readyState === "complete" || document.readyState === "interactive") {
        ...
        if (window.attachEvent) {
            window.attachEvent("onload", setupInicial);
        } else {
            window.addEventListener("DOMContentLoaded", setupInicial, false);
        }
    }
})();
</script>
