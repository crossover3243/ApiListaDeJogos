...
function adicionarLinha() {
    var total_itens = parseInt(document.getElementById('qtd_campos').value) + 1;
    document.getElementById('qtd_campos').value = total_itens;

    // Cria o select vazio com nome esperado
    var selectEl = document.createElement("select");
    selectEl.name = "cod_tipo_doc_campo_" + total_itens;
    selectEl.setAttribute("data-novo", "1");

    // será populado depois com filtro automático

    var coluna_1 = document.createElement("td");
    coluna_1.appendChild(selectEl);

    var coluna_2 = document.createElement("td");
    coluna_2.innerHTML = '<input type="number" name="val_confianca_' + total_itens + '" maxlength="3" onKeyPress="javascript:MascNum();verificaNumero();" onChange="formataConfianca(' + total_itens + ');">%';

    var coluna_3 = document.createElement("td");
    coluna_3.innerHTML = '<input type="number" name="val_peso_' + total_itens + '" maxlength="3" onKeyPress="javascript:MascNum();verificaNumero();" onChange="formataConfianca(' + total_itens + ');">%';

    var coluna_4 = document.createElement("td");
    coluna_4.innerHTML = '<input type="checkbox" name="acao_excluir_' + total_itens + '" onClick="excluirCampo(' + total_itens + ');">' +
                         '<input type="hidden" name="acao_linha_' + total_itens + '" value="I">';

    var tr = document.createElement("tr");
    tr.appendChild(coluna_1);
    tr.appendChild(coluna_2);
    tr.appendChild(coluna_3);
    tr.appendChild(coluna_4);

    var tabela = document.getElementById("tbl_campos_param");
    tabela.appendChild(tr);

    // aciona evento para popular o novo select via script externo
    setTimeout(() => {
